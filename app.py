...
def register():
    ...
    if request.method == "POST":
        ...
        if email and username and pass1 and pass2:
            
            if any([x in username for x in badchars]):
                return render_template('register.html', data=data, message="Caracteres inválidos en campo username")
        
            if any([x in email for x in badchars]):
                return render_template('register.html', data=data, message="Caracteres inválidos en campo email")
           
            if pass1 != pass2:
                return render_template('register.html', data=data, message="Las contraseñas no coinciden")

            cur = mysql.connection.cursor()
            cur.execute("select username from users;")
            all_users = cur.fetchall()[0]
            
            if username in all_users:
                return render_template('register.html', data=data, message="Nombre de usuario en uso")
            
            cur = mysql.connection.cursor()
            cur.execute("select email from users;")
            all_emails = cur.fetchall()[0]
            
            if email in all_emails:
                return render_template('register.html', data=data, message="email ya en uso")
            
            else:
                pass_hash = generate_password_hash(pass1) 
                print(pass_hash)
                cur.execute("INSERT INTO users (username, email, password) VALUES ('{}', '{}', '{}')".format(username, email, pass_hash))
            ...
            return render_template('register.html', data=data, message="Debes rellenar todos los campos")
    else:
...
def login():
    ...
    if request.method == "POST":
        ...
        if email and password:
            ...
            if any([x in email for x in badchars]):
                return render_template('login.html', data=data, message="Caracteres inválidos en campo email")
        
            if any([x in password for x in badchars]):
                return render_template('login.html', data=data, message="Caracteres inválidos en campo password")
            ...
            if result == True:
                session['email'] = email
                cur = mysql.connection.cursor()
                cur.execute("SELECT username from users WHERE email = '{}'".format(email))
...
def chall_entorno():
    ...
    if not 'username' in session:
        message = "Debes iniciar sesión o registrarte para acceder a los retos"
        print("no usuario")
        return render_template('chall_entorno.html', data=data, message=message)  
...
def chall_comandos():
    ...
    if not 'username' in session:
        message = "Debes iniciar sesión o registrarte para acceder a los retos"
        return render_template('chall_comandos.html', data=data, message=message)  
...
def chall_usuarios():
    ...
    if not 'username' in session:
        message = "Debes iniciar sesión o registrarte para acceder a los retos"
        return render_template('chall_usuarios.html', data=data, message=message)  
...
def chall_networking():
    ...
    if not 'username' in session:
        message = "Debes iniciar sesión o registrarte para acceder a los retos"
        return render_template('chall_networking.html', data=data, message=message)  
...
def chall_archivos():
    ...
    if not 'username' in session:
        message = "Debes iniciar sesión o registrarte para acceder a los retos"
        return render_template('chall_archivos.html', data=data, message=message)  
...
def chall_procesos():
    ...
    if not 'username' in session:
        message = "Debes iniciar sesión o registrarte para acceder a los retos"
        return render_template('chall_procesos.html', data=data, message=message)  
